...
def prepare_sequences(data, encoders, secuencia_longitud=40):
...
def train_model(model, X_train, y_train, X_val, y_val, epochs=6000, batch_size=60):
    """Entrena el modelo."""
    try:
        # Cambiar la ruta del archivo para usar una ruta absoluta y nombre simple
        checkpoint_path = os.path.join(os.getcwd(), 'best_model.h5')
...
if __name__ == "__main__":
    ...
    if data is not None:
        ...
        if encoders is not None:
            ...
            model.save("modelo_ruleta_CMulti_Cursor2.h5")
