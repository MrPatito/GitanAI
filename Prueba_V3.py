...
def get_number_characteristics(numero, data):
    """Obtiene las características de un número de la ruleta."""
    # Print debug information
    print(f"Debug - Accessing number {numero} in data:")
    print(f"Data shape: {data.shape}")
    print(f"First few rows of data:\n{data.head()}")
    
    try:
        # Make sure we're accessing the correct row for the number
        row = data[data.index == numero].iloc[0]  # Changed from data.iloc[numero]
        return {
            'color': row['Color'],
            'docena': row['Docena'],
            'fila': row['Fila'],
...
def retrain_model(model, experience_buffer, batch_size=32):
...
caracteristicas_reales = get_number_characteristics(numero_real, data)
...
def train_on_historical_data(model, data, encoders, window_size=40):
